...
function c11111006.filter1(c,e,tp)
	...
	return rk>0 and c:IsFaceup() and c:IsRace(RACE_FAIRY+RACE_SPELLCASTER)
		and Duel.IsExistingMatchingCard(c11111006.filter2,tp,LOCATION_EXTRA,0,1,nil,rk+1,c:GetCode(),e,tp,c)
end
function c11111006.filter2(c,rk,code,e,tp,mc)
...
function c11111006.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c11111006.filter2,tp,LOCATION_EXTRA,0,1,1,nil,tc:GetRank()+1,tc:GetCode(),e,tp,tc)
	local sc=g:GetFirst()
	if sc then
