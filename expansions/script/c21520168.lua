...
function c21520168.ccost(e,tp)
	...
	elseif g1:GetCount()>0 then
		opselect=Duel.SelectOption(tp,aux.Stringid(21520168,0),aux.Stringid(21520168,2))
...
function c21520168.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then 
		return Duel.IsExistingTarget(c21520168.tohandfilter,tp,LOCATION_GRAVE,0,2,e:GetHandler()) 
			or Duel.GetMatchingGroupCount(Duel.IsPlayerCanDraw,tp,LOCATION_DECK,0,nil,tp)>=2 end
	local op=0
	...
	elseif op==2 then
		local g=Duel.SelectTarget(tp,c21520168.tohandfilter,tp,LOCATION_GRAVE,0,2,2,e:GetHandler())
