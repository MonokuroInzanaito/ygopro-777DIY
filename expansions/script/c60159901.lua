...
function c60159901.initial_effect(c)
    ...
    e11:SetCountLimit(1,60159901)
    e11:SetTarget(c60159901.target)
    e11:SetOperation(c60159901.operation3)
...
function c60159901.operation(e,tp,eg,ep,ev,re,r,rp)
    ...
    if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
		...
		local tc=g:GetFirst()
		if (tc:IsAttribute(ATTRIBUTE_EARTH) or tc:IsAttribute(ATTRIBUTE_WATER) or tc:IsAttribute(ATTRIBUTE_WIND))
...
function c60159901.operation3(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
