...
function c11111015.initial_effect(c)
	...
	e1:SetCountLimit(1,11111015+EFFECT_COUNT_CODE_OATH)
...
function c11111015.filter(c,e,tp)
	return c:GetLevel()==8 and c:IsSetCard(0x15d) and c:IsCanBeEffectTarget(e) 
	    and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c11111015.xyzfilter(c,mg)
	if c.xyz_count~=2 then return false end
	return c:IsXyzSummonable(mg) and c:IsSetCard(0x15d)
end
...
function c11111015.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x15d) and c:IsType(TYPE_XYZ) and c:GetRank()>=9
end
function c11111015.mattg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c11111015.cfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c11111015.cfilter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c11111015.cfilter,tp,LOCATION_MZONE,0,1,1,nil)
end	
