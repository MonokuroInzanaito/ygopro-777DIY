...
function c11112026.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		if hd>5 then
		    return Duel.IsPlayerCanDiscardDeck(tp,5) and Duel.IsExistingMatchingCard(c11112026.filter,tp,LOCATION_GRAVE,0,hd,nil)
		else
			return hd>0 and Duel.IsPlayerCanDiscardDeck(tp,hd) and Duel.IsExistingMatchingCard(c11112026.filter,tp,LOCATION_GRAVE,0,hd,nil)
		end
	end
