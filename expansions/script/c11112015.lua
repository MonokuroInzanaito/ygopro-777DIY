...
function c11112015.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsPlayerCanDiscardDeck(tp,1) and Duel.IsExistingTarget(c11112015.filter,tp,LOCATION_MZONE,0,1,nil)
...
function c11112015.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFacedown() or not tc:IsRelateToEffect(e) or tc:IsImmuneToEffect(e) or tc:GetLevel()<2 or Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	local c=e:GetHandler()
	local e1=Effect.CreateEffect(c)
