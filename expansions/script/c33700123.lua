...
function c33700123.initial_effect(c)
	  c:SetUniqueOnField(1,0,33700123)
	 --Activate
...
function c33700123.op(e,tp,eg,ep,ev,re,r,rp)
   ...
   if e:GetHandler():GetFlagEffect(33700123)~=0 then
...
function c33700123.damcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return tc:IsContains(e:GetLabelObject()) and not e:GetLabelObject():IsDisabled() and Duel.GetFieldGroupCount(tp,0,LOCATION_DECK)>0 and e:GetLabelObject():GetFlagEffect(33700123)~=0 and not e:GetHandler():IsDisabled() and re:GetHandler()~=e:GetHandler()
end
