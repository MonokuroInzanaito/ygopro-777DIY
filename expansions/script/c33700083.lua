...
function c33700083.initial_effect(c)
	...
	e2:SetCost(c33700083.cost)
	e2:SetTarget(c33700083.tg)
	e2:SetOperation(c33700083.op)
	c:RegisterEffect(e2)
   --draw
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(33700083,0))
	e3:SetCategory(CATEGORY_DRAW)
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetRange(LOCATION_FZONE)
	e3:SetCountLimit(1) 
	e3:SetCondition(c33700083.thcon)
	e3:SetTarget(c33700083.thtg)
	e3:SetOperation(c33700083.thop)
	c:RegisterEffect(e3)
end
function c33700083.filter(c)
	return c:IsCode(33700175) and c:IsAbleToRemoveAsCost()
end
function c33700083.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c33700083.filter,tp,LOCATION_GRAVE+LOCATION_EXTRA,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c33700083.filter,tp,LOCATION_GRAVE+LOCATION_EXTRA,0,1,1,nil)
...
function c33700083.thcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)==0
end
function c33700083.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local ct=Duel.GetMatchingGroupCount(nil,tp,0,LOCATION_HAND,nil)
	if chk==0 then return ct>0 and Duel.IsPlayerCanDraw(tp,ct) end
	Duel.SetTargetPlayer(tp)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,ct)
end
function c33700083.thop(e,tp,eg,ep,ev,re,r,rp)
