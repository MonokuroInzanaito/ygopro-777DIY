...
function c11111046.filter(c)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0x15d)
end
...
function c11111046.cfilter(c)
	return c:IsFaceup() and (c:IsCode(11111000) or c:IsCode(11111002) or c:IsCode(11111008))
end
function c11111046.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		...
		if Duel.IsExistingMatchingCard(c11111046.cfilter,tp,LOCATION_ONFIELD,0,1,nil) then
		   tc:RegisterFlagEffect(11111046,RESET_EVENT+0x1fc0000+RESET_PHASE+PHASE_END,0,1)
