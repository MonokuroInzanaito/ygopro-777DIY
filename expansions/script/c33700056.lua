...
function c33700056.initial_effect(c)
  ...
  --  e0:SetCondition(c33700056.scon)
	e0:SetOperation(c33700056.sop)
	c:RegisterEffect(e0)
	--Activate
	...
	e1:SetCountLimit(1,33700056+EFFECT_COUNT_CODE_DUEL)
...
function c33700056.target(e,tp,eg,ep,ev,re,r,rp,chk)
 if chk==0 then
		...
		e:SetLabel(0)
		return  Duel.IsExistingMatchingCard(c33700056.filter,tp,LOCATION_HAND,0,1,nil,tp) end 
...
function c33700056.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c33700056.filter,tp,LOCATION_DECK,0,e:GetLabel(),e:GetLabel(),nil)
