--超电磁炮 御坂美琴
function c23306000.initial_effect(c)
	aux.AddXyzProcedure(c,aux.FilterBoolFunction(Card.IsAttribute,ATTRIBUTE_LIGHT),5,3,nil,nil,5)
	c:EnableReviveLimit()
	--destroy
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(23306000,0))
	e1:SetCategory(CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetCost(c23306000.cost)
	e1:SetTarget(c23306000.destg)
	e1:SetOperation(c23306000.desop)
	c:RegisterEffect(e1)
	--handes
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(23306000,1))
	e2:SetCategory(CATEGORY_HANDES+CATEGORY_TOHAND)
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetCost(c23306000.cost)
	e2:SetTarget(c23306000.hdtg)
	e2:SetOperation(c23306000.hdop)
	c:RegisterEffect(e2)
	--deckes
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(23306000,2))
	e3:SetCategory(CATEGORY_DECKDES)
	e3:SetType(EFFECT_TYPE_QUICK_O)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCode(EVENT_FREE_CHAIN)
	e3:SetCost(c23306000.cost)
	e3:SetTarget(c23306000.ddtg)
	e3:SetOperation(c23306000.ddop)
	c:RegisterEffect(e3)
end
function c23306000.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:CheckRemoveOverlayCard(tp,3,REASON_COST) end
	c:RemoveOverlayCard(tp,3,3,REASON_COST)
end
function c23306000.destg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c23306000.desop(e,tp,eg,ep,ev,re,r,rp)
...
function c23306000.hdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)>0	end
	Duel.SetTargetPlayer(1-tp)
	Duel.SetOperationInfo(0,CATEGORY_HANDES,nil,0,1-tp,1)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,0,1-tp,1)
end
function c23306000.hdop(e,tp,eg,ep,ev,re,r,rp)
...
function c23306000.ddtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
...
function c23306000.ddop(e,tp,eg,ep,ev,re,r,rp)
	local gc=Duel.GetFieldGroupCount(tp,0,LOCATION_GRAVE)
