...
function cm.matfilter1(c,syncard,tp)
	...
	if c:IsType(TYPE_XYZ) and c:IsControler(tp) then return true end 
...
function cm.goal(g,tp,lv,syncard,tuc)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)+g:FilterCount(aux.FConditionCheckF,nil,tp)<=0 then return false end
