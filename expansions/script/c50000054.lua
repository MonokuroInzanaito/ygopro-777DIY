...
function c50000054.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(50000054,2))
	e8:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e8:SetType(EFFECT_TYPE_IGNITION)
	e8:SetRange(LOCATION_SZONE)
	e8:SetCountLimit(2)
	e8:SetTarget(c50000054.datg)
	e8:SetOperation(c50000054.daop)
	c:RegisterEffect(e8)
end

function c50000054.dafilter(c)
	return c:IsFaceup() and c:IsSetCard(0x50c) and c:IsType(TYPE_DUAL) and not c:IsDualState()
end
function c50000054.datg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c50000054.dafilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c50000054.dafilter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c50000054.dafilter,tp,LOCATION_MZONE,0,1,1,nil)
end
function c50000054.daop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc and tc:IsRelateToEffect(e) and c50000054.dafilter(tc) then
		tc:EnableDualState()
	end
end
