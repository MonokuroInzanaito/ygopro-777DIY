...
function c21520031.atkval(e,tp,eg,ep,ev,re,r,rp)
	...
	math.randomseed(c:GetFieldID()+Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)+Duel.GetTurnCount()+22360+4)
...
function c21520031.defval(e,tp,eg,ep,ev,re,r,rp)
	...
	local tempmax=c21520031.MaxValue()
	local c=e:GetHandler()
	math.randomseed(c:GetFieldID()+Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)+Duel.GetTurnCount()+22360+5)
	local val=math.random(tempmin,tempmax)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
...
function c21520031.repop(e,tp,eg,ep,ev,re,r,rp)
	...
	local v1,v2=0,0
	math.randomseed(c:GetFieldID()+Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)+Duel.GetTurnCount()+22360+10)
	v1=math.random(0,1024)
	math.randomseed(c:GetFieldID()+Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)+Duel.GetTurnCount()+22360+20)
