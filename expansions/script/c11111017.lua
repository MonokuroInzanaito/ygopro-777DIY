...
function c11111017.initial_effect(c)
	...
	e5:SetDescription(aux.Stringid(11111017,2))
	e5:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e5:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e5:SetCode(EVENT_TO_GRAVE)
	e5:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DELAY)
	e5:SetCondition(c11111017.thcon)
	e5:SetTarget(c11111017.thtg)
	e5:SetOperation(c11111017.thop)
	c:RegisterEffect(e5)
end
...
function c11111017.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	return Duel.SelectYesNo(tp,aux.Stringid(11111017,1))
end
function c11111017.desop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Destroy(e:GetHandler(),REASON_EFFECT+REASON_REPLACE)
end
function c11111017.thcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD)
end
function c11111017.thfilter(c)
	return c:IsSetCard(0x15d) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c11111017.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c11111017.thfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c11111017.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c11111017.thfilter,tp,LOCATION_DECK,0,1,1,nil)
