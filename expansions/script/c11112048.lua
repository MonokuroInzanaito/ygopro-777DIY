...
function c11112048.filter(c)
	return c:IsFaceup() and c:IsCode(11112029) and c:IsCanAddCounter(0xc,1)
end
...
function c11112048.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		local ct=Duel.GetOperatedGroup():FilterCount(c11112048.cfilter,nil)
		if ct>0 then
			if tc:GetCounter(0xc)==9 then
				tc:AddCounter(0xc,1)
			else
				tc:AddCounter(0xc,ct)	  
			end 
		end
	end 
end
