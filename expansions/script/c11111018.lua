...
function c11111018.filter1(c,e,tp)
	return c:IsFaceup() and c:GetRank()==8 and c:IsSetCard(0x15d)
		and Duel.IsExistingMatchingCard(c11111018.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c)
end
function c11111018.filter2(c,e,tp,mc)
	return c:GetRank()==9 and c:IsSetCard(0x15d) and mc:IsCanBeXyzMaterial(c)
...
function c11111018.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,c11111018.filter1,tp,LOCATION_MZONE,0,1,1,nil,e,tp)
...
function c11111018.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c11111018.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc)
	local sc=g:GetFirst()
	if sc then
...
function c11111018.thfilter(c)
	return c:IsType(TYPE_XYZ) and c:IsSetCard(0x15d) and c:IsAbleToRemoveAsCost()
end
