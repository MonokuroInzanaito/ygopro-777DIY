...
function c21520052.initial_effect(c)
	...
	e0:SetCondition(c21520052.fscondition)
	e0:SetOperation(c21520052.fsoperation)
	c:RegisterEffect(e0)
	--spsummon condition
...
function c21520052.fscondition(e,g,gc,chkf)
	if g==nil then return true end
	if gc then return false end
	local g1=g:Filter(Card.IsCode,nil,21520046)
	local g2=g:Filter(Card.IsSetCard,nil,0x494)
	local g3=g:Filter(Card.IsType,nil,TYPE_MONSTER)
	local ag=g1:Clone()
	ag:Merge(g2)
	ag:Merge(g3)
	if chkf~=PLAYER_NONE and not ag:IsExists(aux.FConditionCheckF,1,nil,chkf) then return false end
	return g1:GetCount()>0 and g2:GetCount()>3 and g3:GetCount()>0
end
function c21520052.fsoperation(e,tp,eg,ep,ev,re,r,rp,gc)
	if gc then return end
	local g1=eg:Filter(Card.IsCode,nil,21520046)
	local g2=eg:Filter(Card.IsSetCard,nil,0x494)
	local g3=eg:Filter(Card.IsType,nil,TYPE_MONSTER)
	local ag=g1:Clone()
	ag:Merge(g2)
	ag:Merge(g3)
	local mg=Group.CreateGroup()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	local fg1=ag:FilterSelect(tp,Card.IsCode,1,1,nil,21520046)
	ag:Sub(fg1)
