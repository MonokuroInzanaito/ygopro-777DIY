...
function c11111047.cfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0x15d) and c:IsAbleToRemoveAsCost()
end
...
function c11111047.retop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct==2 then
	    if Duel.ReturnToField(c) then
		   local g=Duel.GetMatchingGroup(Card.IsCode,tp,LOCATION_GRAVE,0,nil,11111047)
