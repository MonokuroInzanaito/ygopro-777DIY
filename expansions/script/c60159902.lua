...
function c60159902.initial_effect(c)
    ...
    e11:SetCountLimit(1,60159902)
    e11:SetTarget(c60159902.target)
    e11:SetOperation(c60159902.operation3)
...
function c60159902.operation(e,tp,eg,ep,ev,re,r,rp)
    ...
    local tc=g:GetFirst()
    if (tc:IsAttribute(ATTRIBUTE_EARTH) or tc:IsAttribute(ATTRIBUTE_FIRE) or tc:IsAttribute(ATTRIBUTE_WIND))
...
function c60159902.operation3(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
