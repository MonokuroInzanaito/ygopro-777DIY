...
function c60150607.filter0(c,e)
	return c:IsFaceup() and c:IsCanBeFusionMaterial() and not c:IsImmuneToEffect(e)
end
function c60150607.filter1(c,e,tp)
	return (c:IsCanBeFusionMaterial() and not c:IsImmuneToEffect(e) and c:IsAbleToGrave()) 
		or (c:IsCanBeFusionMaterial() and not c:IsImmuneToEffect(e) and c:IsType(TYPE_PENDULUM) and c:IsAbleToDeck())
end
...
function c60150607.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg1=Duel.GetMatchingGroup(c60150607.filter1,tp,LOCATION_HAND+LOCATION_MZONE,0,nil,e,tp)
