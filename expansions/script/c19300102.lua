...
function c19300102.synfilter1(c,syncard,lv,g)
	...
	local wg=g:Clone()
	wg:RemoveCard(c)
	return wg:IsExists(c19300102.synfilter2,1,nil,syncard,lv-tlv,wg,t)
end
function c19300102.synfilter2(c,syncard,lv,g,tuner)
	if not c:IsSetCard(0x193) then return false end
	local tlv=c:GetSynchroLevel(syncard)
	if lv-tlv<=0 then return false end
	if not tuner and not c:IsType(TYPE_TUNER) then return false end
	return g:IsExists(c19300102.synfilter3,1,c,syncard,lv-tlv)
end
function c19300102.synfilter3(c,syncard,lv)
...
function c19300102.syncon(e,c,tuner)
	...
	local lv=c:GetLevel()
	if tuner then return c19300102.synfilter1(tuner,c,lv,mg) end
	return mg:IsExists(c19300102.synfilter1,1,nil,c,lv,mg)
end
function c19300102.synop(e,tp,eg,ep,ev,re,r,rp,c,tuner)
	...
	if not tuner then
		...
		local t1=mg:FilterSelect(tp,c19300102.synfilter1,1,1,nil,c,lv,mg)
		m1=t1:GetFirst()
		g:AddCard(m1)
	end
	...
	mg:RemoveCard(m1)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SMATERIAL)
	local t2=mg:FilterSelect(tp,c19300102.synfilter2,1,1,nil,c,lv,mg,t)
	local m2=t2:GetFirst()
	g:AddCard(m2)
	lv=lv-m2:GetSynchroLevel(c)
	mg:RemoveCard(m2)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SMATERIAL)
	local t3=mg:FilterSelect(tp,c19300102.synfilter3,1,1,nil,c,lv)
	g:Merge(t3)
	c:SetMaterial(g)
