...
function c121029233.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	return Duel.SelectYesNo(tp,aux.Stringid(23310009,10))
end
...
function c121029233.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tf(re,rp,ceg,cep,cev,cre,cr,crp,0,g:GetFirst()) and Duel.SelectYesNo(tp,aux.Stringid(23310009,11)) then
	   Duel.BreakEffect()
...
function c121029233.cbop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c121029233.spfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
	if g:GetCount()>0 and Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)>0 then 	
	if not Duel.GetAttacker():IsImmuneToEffect(e) and Duel.SelectYesNo(tp,aux.Stringid(23310009,11)) then
		   Duel.BreakEffect()
